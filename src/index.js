...
const io = socketio(server) // Express requires the server created above behind the scenes
...
io.on('connection', (socket) => {
    ...
    socket.on('join', (options, callback) => {
        ...
        callback()    
        // socket.emit, io.emit, socket.broadcast.emit -> For all
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            io.to(user.room).emit('message', generateMessage("Moobot", `${user.username} has left`))
            io.to(user.room).emit('roomData', {
                room: user.room,
                users: getUsersInRoom(user.room)
            })
